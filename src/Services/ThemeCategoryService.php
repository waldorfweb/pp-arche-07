 ...
 *
 */
class ThemeCategoryService extends CategoryService
{
    use MemoryCache;
    ...
    {
        parent::__construct($categoryRepository, $webstoreConfigurationRepository, $authGuard, $contactRepository, $urlBuilderRepository, $webshopCategoryRepositoryContract);
    ...
    public function getPartialTreeWithDepth($categoryId = 0, $type = CategoryType::ALL, $depth = 1)
    {
        $this->treeDepth = $depth;
        try {
            $return = $this->getPartialTree($categoryId, $type);
        } catch (\Exception $e) {
        }
        $this->treeDepth = 1;
        return $return;
    ...
    public function getPartialTree($categoryId = 0, $type = CategoryType::ALL)
    {
        if ($categoryId > 0) {
            ...
            foreach ($tree as $i => $category) {
                $this->appendBranchFields($tree[$i], $siblingCount, '', $this->treeDepth);
            }
